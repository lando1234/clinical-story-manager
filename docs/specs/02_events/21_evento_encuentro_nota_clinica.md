# Sistema de Registros Médicos Psiquiátricos — Especificación Funcional: Separación Evento NOTE / Nota Clínica
...
- **Evento NOTE** (ClinicalEvent de tipo NOTE)
- **Documento Nota Clínica** (Note)

**Nota importante:** Este documento se refiere a eventos NOTE generados por Notes finalizadas. Los eventos Encounter son un tipo diferente generado por Appointments (ver `23_encounter_appointment_spec.md`).
...
## 1. Definición de Evento NOTE

### 1.1 Qué es un Evento NOTE

Un **Evento NOTE** es una ocurrencia clínicamente significativa que representa que una Nota Clínica fue finalizada y documentada.
...
### 1.3 Propósito Clínico del Evento NOTE
...
### 1.4 Lo que NO es un Evento NOTE
...
- **NO es un evento Encounter** — Los eventos Encounter son generados por Appointments, no por Notes
...
Una **Nota Clínica** es el documento que contiene la documentación estructurada de un Encuentro clínico entre el clínico y el paciente.

**Características esenciales:**

- Es una instancia de la entidad `Note`
- Contiene el contenido clínico del Encuentro clínico (subjetivo, objetivo, evaluación, plan)
...
| **encounterType** | Enumeration | Tipo de Encuentro clínico (evaluación inicial, seguimiento, etc.) | Inmutable (una vez finalizada) |
...
1. **Documentar el contenido clínico** — Captura las observaciones, hallazgos, evaluaciones y planes del Encuentro clínico
...
## 3. Relación entre Evento NOTE y Nota Clínica
...
- El hecho del Encuentro clínico ya fue registrado cuando la Nota fue finalizada
...
- **Eventos Encounter** (ClinicalEvent con eventType = Encounter, generados por Appointments)
...
### 4.2 Por Qué Aparece el Evento NOTE

**Razones clínicas:**

1. **Narrativa longitudinal** — La Timeline muestra la secuencia de eventos clínicamente significativos
2. **Navegación temporal** — Permite ubicar Encuentros clínicos documentados en el tiempo del paciente
3. **Correlación con otros eventos** — Facilita ver Encuentros clínicos junto con cambios de medicación, hospitalizaciones, etc.
4. **Completitud histórica** — Garantiza que todos los Encuentros clínicos documentados aparezcan en la Timeline
...
1. **Evento NOTE con Nota Clínica**
   - El Evento es el hecho histórico
   - La Nota es el documento clínico
   - Son entidades diferentes con propósitos diferentes

2. **Evento NOTE con Evento Encounter**
   - Evento NOTE = generado por Notes finalizadas (documentación clínica)
   - Evento Encounter = generado por Appointments pasados (turnos agendados)
   - Son tipos de eventos diferentes con fuentes diferentes

3. **Fecha del evento con fecha de creación de la nota**
   - `eventDate` del Evento = fecha en que ocurrió el Encuentro clínico
   - `createdAt` de la Nota = fecha en que se creó el documento
   - Pueden ser diferentes (documentación tardía)

4. **Finalización de Nota con creación de Evento**
   - Finalizar una Nota es una acción del clínico
   - Crear el Evento NOTE es una consecuencia automática
   - No son la misma operación

5. **Edición de Nota Draft con modificación de Evento**
...
6. **Addenda con nuevos eventos**
   - Los Addenda son parte de la Nota
   - Los Addenda NO generan nuevos Eventos NOTE
   - Los Addenda NO aparecen como eventos separados en la Timeline
...
5. **Permitir eliminación de Eventos NOTE**
   - ❌ Incorrecto: Botón "Eliminar" en eventos NOTE
   - ✅ Correcto: Eventos son permanentes, no se pueden eliminar

6. **Confundir Evento NOTE con Evento Encounter**
   - ❌ Incorrecto: Tratar eventos NOTE y Encounter como si fueran el mismo tipo
   - ✅ Correcto: Reconocer que son tipos diferentes: NOTE (de Notes) y Encounter (de Appointments)

### 5.3 Garantías de Separación

**El sistema debe garantizar:**

1. **Identidad distinta** — Evento NOTE y Nota Clínica tienen identificadores únicos diferentes
...
### 6.1 Inmutabilidad del Evento NOTE

**Desde el momento de creación, el Evento NOTE es completamente inmutable:**

| Atributo | Inmutabilidad | Razón |
|----------|---------------|-------|
| **id** | Inmutable | Identificador único permanente |
| **eventDate** | Inmutable | Fecha histórica del Encuentro clínico no puede cambiar |
...
- La inmutabilidad del Evento NOTE **NO depende** de la inmutabilidad de la Nota
...
|---------|----------------------|
| **Evento NOTE** | Preserva el hecho histórico de que se documentó un Encuentro clínico |
...
- El Evento NOTE **NO tiene estados**
...
### 7.1 Presentación del Evento NOTE en la Timeline

**Elementos semánticos que deben aparecer:**

1. **Indicador de tipo** — Debe ser claro que es un "Evento NOTE" o "Nota Clínica"
2. **Fecha del Encuentro clínico** — Fecha clínica (eventDate), no fecha de registro
3. **Título descriptivo** — Resumen breve del Encuentro clínico
...
3. **Fecha de creación de la Nota** — No debe confundirse con la fecha del Encuentro clínico
...
2. **Fecha del Encuentro clínico** — Debe coincidir con eventDate del Evento asociado
3. **Tipo de Encuentro clínico** — Debe ser visible y claro
...
1. **Información del Evento NOTE** — No debe duplicarse información que ya está en la Timeline
...
|----------|--------------|----------|
| Evento en Timeline | "Nota Clínica" o "Evento NOTE" | Timeline, lista de eventos |
...
- Debe haber un **indicador visual o textual** que identifique el tipo de evento (NOTE)
- Debe haber un **mecanismo de acceso** al documento Nota asociado
- Debe ser **distinguible** de otros tipos de eventos (Encounter, medicación, hospitalización, etc.)
...
- **Con separación:** El `eventDate` del Evento NOTE es independiente e inmutable, preservando el orden histórico
...
- **Con separación:** El Evento NOTE es independiente y no puede eliminarse, preservando la historia clínica
...
- **Con separación:** La transición Draft → Finalized es única e irreversible, garantizando un solo evento NOTE por Nota
...
- **Con separación:** Solo los Eventos NOTE (de Notas finalizadas) aparecen en la Timeline
...
- **Con separación:** El Evento NOTE preserva el hecho histórico; la Nota preserva la documentación original
...
- **Con separación:** La generación automática de eventos NOTE garantiza que toda Nota finalizada tenga su evento correspondiente
...
- **Con separación:** La inmutabilidad de Notas finalizadas y Eventos NOTE preserva la integridad legal
...
- **Con separación:** Los Eventos NOTE son permanentes e independientes de las Notas
...
- **Usa el tipo de evento `NOTE` existente** — No requiere nuevos tipos de evento
- **No modifica reglas de generación** — Respeta el mecanismo de generación automática al finalizar Notas
- **No cambia reglas de ordenamiento** — Los Eventos NOTE siguen las mismas reglas de ordenamiento que otros eventos
...
1. **Completitud** — Toda Nota finalizada genera exactamente un Evento NOTE
2. **Exclusión** — Notas en Draft no aparecen en la Timeline
3. **Inmutabilidad** — Los Eventos NOTE son inmutables como todos los eventos
4. **Ordenamiento** — Los Eventos NOTE se ordenan según las reglas del Timeline Engine
...
- **WRITE-EVENT-NOTE** — Generación de Evento NOTE al finalizar Nota
- **IMMUTABLE-EVENT** — Inmutabilidad de todos los atributos del Evento NOTE
- **READ-EVENT-SOURCE** — Acceso a la Nota desde el Evento NOTE
- **TIMELINE-ORDERING** — Ordenamiento de Eventos NOTE en la Timeline
...
- `eventType = NOTE`, `sourceType = Note`
...
**Separación con Evento Encounter:**
- Los eventos Encounter son un tipo diferente generado por Appointments (turnos agendados)
- Los eventos NOTE y Encounter son independientes y tienen fuentes diferentes
- Ver `23_encounter_appointment_spec.md` para detalles sobre eventos Encounter

### 10.2 Principios Clave

1. **Un Encuentro clínico documentado, dos representaciones** — El hecho (Evento NOTE) y el documento (Nota) son entidades distintas
...
4. **Inmutabilidad por razones diferentes** — Evento NOTE preserva historia, Nota preserva documentación legal
5. **Presentación diferenciada** — La UI debe distinguir claramente entre evento y documento
6. **Separación de tipos** — Evento NOTE (de Notes) y Evento Encounter (de Appointments) son tipos diferentes

### 10.3 Garantías del Sistema

- **Integridad histórica** — Los Encuentros clínicos documentados no pueden eliminarse de la Timeline
