...
|-------------|-----------|-------------|
| Note | NOTE event | A finalized clinical note produces exactly one NOTE event. |
...
The initial event marking the creation of a ClinicalRecord and the formal start of the documented clinical history.

**Generated By:**

Creation of a ClinicalRecord entity.

**When Created:**

Automatically when a ClinicalRecord is created for a patient.

**Event Date Source:**

The creation date of the ClinicalRecord (or patient registration date).

**Clinical Significance:**

Ensures the timeline is never empty and provides a clear starting point for the patient's documented clinical history.

**Event Type:** `Foundational`  
**Source Type:** `null` (no source entity)

**Priority:** 0 (highest priority, always appears first)

### 3.1 NOTE

**Meaning:**

An Encuentro clínico (clinical encounter) has been documented through a finalized Note.
...
Represents documented clinical encounters—the core of psychiatric care documentation.
...
- [`22_nota_clinica_evento_note.md`](../02_events/22_nota_clinica_evento_note.md) — Defines the separation between Nota Clínica (Note entity) and Evento NOTE (timeline event), and establishes that NOTE events are generated by finalized Notes.
- [`21_evento_encuentro_nota_clinica.md`](../02_events/21_evento_encuentro_nota_clinica.md) — Defines the separation between Evento de Encuentro and Nota Clínica, providing context for the distinction between clinical encounters and their documentation.

### 3.2 Encounter

**Meaning:**

A scheduled Turno agendado (appointment) has been scheduled or has occurred.

**Generated By:**

An Appointment entity, regardless of whether its scheduled date is in the future or past.

**When Created:**

Immediately when an appointment is created/scheduled, regardless of the scheduled date.

**Event Date Source:**

The scheduled date of the Appointment (may be in the future).

**Clinical Significance:**

Represents scheduled Turnos agendados (appointments), providing temporal markers for administrative tracking. Events with future dates exist in the database but are filtered from timeline display until their scheduled date passes.

**Event Type:** `Encounter`  
**Source Type:** `Appointment`

**Note:** This is different from NOTE events. NOTE events represent documented Encuentros clínicos (clinical encounters), while Encounter events represent scheduled Turnos agendados (appointments). See [`09_terminology_glossary.md`](../00_foundation/09_terminology_glossary.md) for standard terminology.

**Related Specifications:**

- [`23_encounter_appointment_spec.md`](../02_events/23_encounter_appointment_spec.md) — Defines Encounter events derived from Appointments (scheduled appointments), including rules for creation, visibility, and handling of future-dated events.

### 3.3 Medication Start
...
The prescription issue date recorded in the Medication.

**Clinical Significance:**

Marks the beginning of a pharmacological intervention.

**Event Type:** `Medication Start`  
**Source Type:** `Medication`

### 3.4 Medication Prescription Issued

**Meaning:**

A new prescription has been issued for an active medication without modifying posological parameters (dosage, frequency, unit).

**Generated By:**

Manual action by a clinician to issue a new prescription for an existing active Medication.

**When Created:**

When a clinician explicitly issues a new prescription for a medication that is already active in the patient's record.

**Event Date Source:**

The prescription issue date provided by the clinician (must be after the original medication's prescription_issue_date).

**Clinical Significance:**

Documents prescription renewals and continuity of treatment without dosage modifications. Differentiates between "issuing a new prescription" and "adjusting dosage" as distinct clinical actions.

**Event Type:** `MedicationPrescriptionIssued`  
**Source Type:** `Medication`

**Special Rules:**

- Can only be issued for medications with status=Active
- The prescription issue date must be after the original Medication.prescription_issue_date
- Does not modify the Medication entity (unlike Medication Change)
- Does not create a new Medication version

### 3.5 Medication Change
...
The prescription issue date of the new Medication record (which is the effective date of the change).

**Clinical Significance:**

Documents titration, dose adjustments, and frequency modifications as part of treatment optimization.

**Event Type:** `Medication Change`  
**Source Type:** `Medication`
...
NOTE events do not have a fixed priority. They are ordered purely by their timestamps (event timestamp, then recorded timestamp, then identifier). NOTE events appear in chronological order relative to other events based on their occurrence date, without priority-based positioning.

**Priority Order (for non-NOTE events):**

| Priority | Event Type |
|----------|------------|
| 0 (highest) | Foundational |
| 2 | Encounter |
| 3 | Medication Start |
| 4 | Medication Change |
...
NOTE events are ordered chronologically without priority constraints, allowing them to appear at their natural position in the timeline based on when the clinical encounter occurred.
...
Events with event timestamps in the future are not permitted, **with one exception: Encounter events**.

The Timeline represents what has occurred, not what is planned.

**Exception: Encounter Events**

Encounter events may be created with future timestamps when appointments are scheduled. These events are persisted in the database but are filtered from timeline display until their event timestamp has passed. This exception allows the system to maintain consistency between appointments and their corresponding events while preserving the principle that the timeline only shows past occurrences.

**Enforcement:**

- Event creation must validate that event timestamp ≤ current date, **except for Encounter events**.
- Encounter events with future timestamps are created but filtered from timeline queries.
- Planned future actions (other than scheduled appointments) are documented in encounter note Plan sections, not as timeline events.
...
- Include medications where prescription_issue_date ≤ D AND (end_date IS NULL OR end_date > D).
