...
The initial event that marks the creation of a ClinicalRecord and the formal start of the documented clinical history.

This event is automatically created when a ClinicalRecord is created for a patient.

It has the highest priority (0) in event ordering, ensuring it always appears first in the timeline.

The timeline is never empty as long as a ClinicalRecord exists.

**NOTE Events**

Direct clinical contact between the clinician and patient that has been documented.
...
Scheduled appointments (Turnos agendados) that have occurred.

These events are generated when an appointment's scheduled date has passed.

Examples: scheduled follow-up that occurred, initial evaluation appointment that took place.

**Note:** NOTE events represent documented Encuentros clínicos (clinical encounters), while Encounter events represent scheduled Turnos agendados (appointments) that occurred. They are different event types with different sources. See [`09_terminology_glossary.md`](../00_foundation/09_terminology_glossary.md) for standard terminology.

**Medication Events**

Changes to the patient's pharmacological treatment.

These events mark the start, modification, prescription renewals, or discontinuation of medications.

Examples: starting an antidepressant, increasing dosage, issuing a new prescription for an active medication, stopping a medication due to side effects.
...
A finalized Note generates exactly one NOTE event.
...
**Note:** This is different from Encounter events, which are generated by Appointments (Turnos agendados). See [`09_terminology_glossary.md`](../00_foundation/09_terminology_glossary.md) for standard terminology. (See Appointment → Encounter Event below.)
...
#### Appointment → Encounter Event

An Appointment generates exactly one Encounter event immediately upon creation, regardless of whether the scheduled date is in the future or past.

The event is created automatically when the appointment is scheduled.

Future appointments generate Encounter events that exist in the database but are hidden from the timeline until the scheduled date passes.

The event date matches the scheduled date of the appointment.

**Note:** This is different from NOTE events, which are generated by finalized Notes. Encounter events represent scheduled Turnos agendados (appointments) that occurred, while NOTE events represent documented Encuentros clínicos (clinical encounters). See [`09_terminology_glossary.md`](../00_foundation/09_terminology_glossary.md) for standard terminology.
...
The ordering priority is: Encounter, Medication Start, Medication Prescription Issued, Medication Change, Medication Stop, Hospitalization, Life Event, History Update, Other.

This ensures Encuentro clínico documentation (NOTE events) appears before related medication changes on the same day.
...
Encounter events are an exception to this rule. Encounter events may be created with future dates when appointments are scheduled, but they are filtered from timeline display until their occurrence date has passed. This allows the system to maintain consistency between appointments and their corresponding events while preserving the principle that the timeline only shows past occurrences.

**General Rule for Other Event Types:**

Planned future actions (such as intended medication starts) are not timeline events.

For all event types except Encounter, an event enters the timeline only when its occurrence date has passed.
